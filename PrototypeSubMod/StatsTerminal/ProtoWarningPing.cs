...
internal class ProtoWarningPing : MonoBehaviour
{
    ...
    private void Start()
    {
        if (warningType == CyclopsHolographicHUD_WarningPings.WarningTypes.Damage)
        {
            ...
            Vector3 parentOffset = (transform.position - parent.position).normalized;
            Vector3 dotPos = transform.position + parentOffset * 0.35f;
            labelDot.transform.position = dotPos;
